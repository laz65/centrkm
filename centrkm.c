...
a = a + ton1;      // ‘ормирование синусоиды (форма в массиве sinus)
z = a>>8 ;
OCR0B=sinus[z];
...
if(per_old < per_new) period = per_new - per_old; else period = per_new + (255 - per_old);  
    if ((period > 0x80)&&(period < 0xA0)) //если период соответствует частоте 435 √ц
    {
...
{
    long koeff;              //определение коеффициента делени€ дл€ получени€ частоты
    int delit;
...
ACSR |= (1<<ACIE); // включить перывани€ от компарат
...
#asm("sei")
    ...
    {   
      ...
      {     
          ...
          while (time < 6);
          ACSR &= ~(1<<ACIE); // выкл прер компар  
          if (flag435) // если был отлет 435 √ц
          { 
            switch (i)
            {
                case 31:  
